...
### macOS
```bash
curl -fsSL https://raw.githubusercontent.com/lifeistech/lit-corporate/main/install.sh | bash
```
途中で Mac のログインパスワードを求められることがあります（Homebrew 新規導入時）。

### Windows (PowerShell)
```powershell
iwr -useb https://raw.githubusercontent.com/lifeistech/lit-corporate/main/install.ps1 | iex
```
...
3. `~/lit-corporate` にテンプレートを clone
...
node setup.js \
  --target /tmp/lit-test \
  --depts accounting,labor,general-affairs,hr,recruit \
  --model sonnet \
  --commands \
  --gitignore \
  --non-interactive
...
非対話モードでは自動起動 (`claude` の spawn) は既定で **off**。有効化したい場合は `--launch-claude` を追加。逆に対話モードで起動をスキップしたい場合は `--no-launch-claude`。

## 単体テスト例

```bash
# ステータスライン動作確認
echo '{"model":{"display_name":"Opus"},"context_window":{"used_percentage":24,"context_window_size":200000}}' | node templates/statusline.js

# 機密検知フック動作確認
echo '{"prompt":"sk-ant-abc123def456ghi789jkl"}' | node templates/hooks/secret-guard.js; echo "exit=$?"  # → exit=2
```

## アップデート

```bash
cd ~/lit-corporate && git pull
...
- 変更後は `main` に merge すれば、各利用者は `cd ~/lit-corporate && git pull && node setup.js` で取り込める
- 従業員向け案内には README 冒頭のワンライナーを貼ればよい（`lifeistech/lit-corporate` が参照される）
